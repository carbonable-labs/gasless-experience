import { BASE_URL, GaslessOptions, RequestError, SEPOLIA_BASE_URL } from "@avnu/gasless-sdk";
import { ec, hash } from "starknet";

const url = process.env.NEXT_PUBLIC_NETWORK === 'mainnet' ? BASE_URL : SEPOLIA_BASE_URL
export const baseUrl = (options?: GaslessOptions): string => options?.baseUrl ?? url;
...
export const securedPostRequest = (body: unknown, apiKey: string, options?: GaslessOptions): RequestInit => ({
  method: 'POST',
  headers: {
    Accept: 'application/json',
    'Content-Type': 'application/json',
...
export const parseResponse = async <T>(response: Response, apiPublicKey?: string): Promise<T> => {
